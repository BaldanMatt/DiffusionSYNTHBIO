...
import matplotlib
import pathlib, os
def load_data(metadata_path, data_path, dim=1000, verbose: bool =False):
...
def validate_and_clean(metadata, data, verbose: bool = False):
    ...
    if verbose:
        ...
        for i in range(data["y"].shape[1]):
            label_indices = np.where(data["y"][:, i] == 1)[0]
            print(
                f"\tLabel {i}: {len(label_indices)} samples in data, {len(metadata.iloc[label_indices])} samples in metadata.")
...
def exploratory_data_analysis(metadata, data, interactive: bool = False, verbose: bool = False):
