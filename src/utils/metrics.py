...
def count_ngrams(one_hot: np.ndarray, n: int):
    ...
    base_values = (5 ** np.arange(vocab_size))[
        ::-1
    ]  # Example: [5^4, 5^3, 5^2, 5^1, 5^0]
    ...
    encoded_ngrams = np.dot(ngrams, base_values).sum(
        axis=-1
    )  # Shape: (Nobs, seq_len - n + 1)
...
def numba_compute_hamming_distance(ref, gen):
    ...
    for i in numba.prange(N):
        for j in range(M):
            dist_matrix[i, j] = (
                np.sum(ref[i] != gen[j]) / ref.shape[1]
            )  # Hamming distance
...
@numba.njit(parallel=True)
def numba_filter_pairs(ref, gen, threshold=0.3):
    N, _ = ref.shape
    M, _ = gen.shape
    filtered_pairs = List()
    for i in numba.prange(N):
        for j in range(M):
            dist = np.sum(ref[i] != gen[j]) / ref.shape[1]
            if dist < threshold:
...
def numba_compute_lcs_lowmemory(ref_indices, gen_indices):
    # Filter pairs based on a distance threshold
    filtered_pairs = numba_filter_pairs(ref_indices, gen_indices)
...
def compute_lcs(ref, gen):
    ...
    for i, j in filtered_pairs:
        lcs_results[(i, j)] = longest_alignment(
            ref_indices[i], gen_indices[j], max_seq_length
        )
    return lcs_results
...
def numba_longest_alignment(A, B):
    ...
    for _ in range(m + 1):
        ...
        )  # Inner lists are of type List(int32)
...
######## MOTIF CORRELATION ########
def find_subsequence(one_hot, subsequence: str, char_list: str):
...
def motif_correlation(
    one_hot1,
    one_hot2,
    motif: str = "TATAWAW",
    onehot: str = "ACGTN",
    figname: str = "motif_correlation.png",
):
    distr1 = (
        find_subsequence(one_hot1, motif, onehot).mean(axis=0)
        if isinstance(one_hot1, np.ndarray)
        else find_subsequence(one_hot1, motif, onehot).float().mean(dim=0)
    )
    distr2 = (
        find_subsequence(one_hot2, motif, onehot).mean(axis=0)
        if isinstance(one_hot2, np.ndarray)
        else find_subsequence(one_hot2, motif, onehot).float().mean(dim=0)
    )
    ...
    if isinstance(one_hot1, np.ndarray) and isinstance(one_hot2, np.ndarray):
        ...
        raise TypeError(
            "Both inputs must be of the same type, either numpy arrays or torch tensors"
        )
...
#
        ...
        # lcs_results = numba_compute_lcs(ref_indices, gen_indices)
        lcs_results = numba_compute_lcs_lowmemory(ref_indices, gen_indices)
